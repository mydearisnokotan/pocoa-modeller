...
export default function Workspace() {
  ...
  const pickFile = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    setFileError("");
    if (!selected) return;
    if (!/^image\/(png|jpeg)$/.test(selected.type)) { setFileError("PNGまたはJPEG形式の画像を選択してください。"); return; }
  ...
  return <div className="min-h-screen bg-[#090d20] text-slate-100"><AppHeader /><main className="mx-auto max-w-[1440px] px-4 py-8 sm:px-6 lg:px-8">
    ...
    <div className="grid gap-6 xl:grid-cols-[1.05fr_.95fr]">
      <section className="pixel-panel p-5 sm:p-7"><div className="flex items-center gap-2 text-xs font-bold tracking-[.16em] text-cyan-200"><UploadCloud className="h-4 w-4" /> STEP 01 / UPLOAD</div><div className="mt-6 space-y-5"><div className="space-y-2"><Label>プロジェクト名</Label><Input value={title} onChange={event => setTitle(event.target.value)} className="border-white/15 bg-[#090d20] text-white" /></div><div className="space-y-2"><div className="flex items-center justify-between"><Label>建築の高さ</Label><span className="font-pixel text-xs text-lime-300">{height} blocks</span></div><input aria-label="建築の高さ" type="range" min="20" max="500" step="10" value={height} onChange={event => setHeight(Number(event.target.value))} className="h-2 w-full cursor-pointer accent-lime-300" /><div className="flex justify-between text-[10px] text-slate-500"><span>20</span><span>100</span><span>500</span></div></div>
        <div className="space-y-2"><Label>建築資料画像 <span className="text-slate-500">PNG / JPEG・5MBまで</span></Label><label className="group flex min-h-64 cursor-pointer flex-col items-center justify-center border-2 border-dashed border-cyan-300/30 bg-[#090d20] p-5 text-center transition-colors hover:border-lime-300/70 hover:bg-lime-300/5">{dataUrl ? <img src={dataUrl} alt="アップロードした建築資料" className="max-h-56 max-w-full object-contain" /> : <><span className="grid h-14 w-14 place-items-center border border-cyan-300/30 bg-cyan-300/10 text-cyan-200"><ImagePlus className="h-7 w-7" /></span><p className="mt-4 text-sm font-bold text-slate-100">画像を選択</p><p className="mt-2 text-xs text-slate-500">クリックしてファイルをアップロード</p></>}<input type="file" accept="image/png,image/jpeg" onChange={pickFile} className="sr-only" /></label>{file && <p className="flex items-center gap-2 text-xs text-lime-200"><CheckCircle2 className="h-4 w-4" />{file.name}</p>}{fileError && <p className="flex items-center gap-2 text-xs text-fuchsia-200"><AlertCircle className="h-4 w-4" />{fileError}</p>}</div>
