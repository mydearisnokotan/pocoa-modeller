...
type BlueprintResult = { blueprint2d: Blueprint2DData; blueprint3d: Blueprint3DData; blockSummary: Array<{ blockId: number; blockName: string; colorHex: string; count: number }>; materialSummary: Array<{ materialId: number; name: string; count: number; description: string | null; locationName: string | null; locationDescription: string | null; locationImageUrl: string | null }>; totalBlocks: number };

function savedBlueprint(value2d: unknown, value3d: unknown): BlueprintResult | null {
  if (!value2d || !value3d || typeof value2d !== "object" || typeof value3d !== "object") return null;
  const twoD = value2d as Partial<Blueprint2DData & BlueprintResult>;
  const threeD = value3d as Partial<Blueprint3DData>;
  if (!Array.isArray(twoD.cells) || !Array.isArray(twoD.legend) || !Array.isArray(threeD.voxels) || typeof twoD.gridSize !== "number" || typeof threeD.gridSize !== "number") return null;
  return {
    blueprint2d: { gridSize: twoD.gridSize, cells: twoD.cells, legend: twoD.legend },
...
export default function DesignStudio() {
  ...
  useEffect(() => {
    ...
    const previous = savedBlueprint(project.data?.blueprint2d, project.data?.blueprint3d);
